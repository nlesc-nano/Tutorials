...
An important point here is that CAT automatically browse the provided ligand for the following functional groups: xxxx, xxxx, . If there is more than one present, e.g. 3, then CAT will build 3 QD models with the ligands bound from different anchoring groups. 
